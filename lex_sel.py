...
def gen_rules():
    rules = Counter()
    for key, count in ambiguity.most_common(args.lemma_count * (skip_count + 1))[-args.lemma_count:]:
        for w, c in lemma_index[key]:
            for r in gen_rules_window(w, c):
                rules[(key, r)] += 1
    return rules
...
with (TemporaryDirectory() as tmpdir,
    ...
    for iteration in range(args.iterations):
        rules = gen_rules()
        scored_rules = []
        for i, ((k, r), c) in enumerate(rules.most_common(args.rule_count)):
        ...
        print(f'## {iteration+1}:', sum(base_scores), f'{skip_count=}')
