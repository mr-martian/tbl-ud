...
EXCLUDE = set(json.loads(args.exclude))
...
def gen_rules(window, slw, tlw):
    ...
    rules = [r for r in rules if error_key(r) not in EXCLUDE]
    return rules
...
with TemporaryDirectory() as tmpdir:
    ...
    with (open(gpath, 'w') as fout,
          open(f'grammar_round9_{args.fold}.cg3', 'a') as fout2):
        fout.write('OPTIONS += addcohort-attach ;\n')
        fout.write('DELIMITERS = "<$$$>" ;\n')
        fout.write('PROTECT (SOURCE) ;\n')
    ...
    print(json.dumps(list(failed_errors - non_failed)), file=sys.stderr)
